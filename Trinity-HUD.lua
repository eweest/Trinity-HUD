...
function main()
	...
	sampRegisterChatCommand("hud", function(cmd)
		...
		elseif cmd == "reset" then -- REMOVE SETTINGS
			...
			sendToChatMsg(string.format("Вы сбросили файл с настройками ресурса %s.", SCRIPT.name), "done", true)
			reloadScript()
...
function createHUD()
	...
	if DB[GET_SERVER]["VIEW"] then
		...
		if getPlayerMoney() == 0 then
			dxDrawText("На руках нет денег", sX-hudX + 486, hudY + 209, FONTS["hud-16"], "right", 0x40FFFFFF) -- TEXT (MONEY)
		else
...
-- <FUNCTION> DELETE STANDART TEXTDRAW
function removeServerTextdraw(td)
	if td == "time" then
		sampTextdrawDelete(2060) -- TIME
		sampTextdrawDelete(2061) -- DATE
	end
	return td
end
