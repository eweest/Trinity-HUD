...
assert(lib, "{FF3232}Отсутствует библитека \"memory\"")
local lib, weapons = pcall(require, "game.weapons")
assert(lib, "{FF3232}Отсутствует библитека \"game.weapons\"")
...
local wanted_reason = {
	{"Агент {ffffff}%a+_%a+{6495ED} снял вам розыск."},
	{"Агент {ffffff}%a+_%a+{6495ED} сняла вам розыск."},
	{"Ваш уровень розыска в {FF8282}%u%u{ffffff} снизился."},
	{"Ваш уровень розыска в {FF8282}%u%u{ffffff} снизился. Вы больше не разыскиваетесь в этом государстве."},
	{"Тюремщик:{ffffff} Всё, ты свободен. Надеюсь больше не видеть тебя."},
}

local wanted_unreason = {
	{"Вы погибли при задержании от руки сотрудника RC FBI по имени {abcdef}%a+_%a+{ffffff}."},
	{"Тюремщик:{ffffff} Добро пожаловать в тюрьму {D8A903}Алькатрас{ffffff}"},
	{"Тюремщик:{ffffff} С возвращением в тюрьму {D8A903}Алькатрас{ffffff}"},
	{"Тюремщик:{ffffff} Вы проведете в изоляторе этого полицейского участка"},
	{"Агент {ffffff}%a+_%a+{6495ED} выпустил вас на свободу."},
	{"Агент {ffffff}%a+_%a+{6495ED} выпустила вас на свободу."},
}

local jailtime = {
	["reason"] = {
		"Остаток срока вашего заключения в департаменте полиции %a+ %a+ составляет {D8A903}",
		"Остаток срока вашего заключения в тюрьме «Алькатрас» составляет {D8A903}",
		"Eweest_Rigz сказал: Остаток срока вашего заключения в тюрьме «Алькатрас» составляет ",
	},
	["jail"] = {
		"Тюремщик:{ffffff} Вы проведете в изоляторе этого полицейского участка ",
		"Тюремщик:{ffffff} Добро пожаловать в тюрьму {D8A903}Алькатрас{ffffff}, вы проведете у нас ",
	}
}
-- <FUNCTION> Jail Timer
...
function sampev.onServerMessage(color, text)
	...
	for _, reason in ipairs(jailtime["reason"]) do 
		if text:find(reason) then
			...
			if text:match(reason .. "(%d+) ч. (%d+) мин. (%d+) сек.") then
				local hour, min, sec = text:match(reason .. "(%d+) ч. (%d+) мин. (%d+) сек.")
				jailTimerEnd = os.time() + (timeZone + hour * 3600 + min * 60 + sec)
				jailTimerState = true
				return false
			end
			if text:match(reason .. "(%d+) мин. (%d+) сек.") then
				local min, sec = text:match(reason .. "(%d+) мин. (%d+) сек.")
				jailTimerEnd = os.time() + (timeZone + min * 60 + sec)
				jailTimerState = true
				return false
			end
			if text:match(reason .. "(%d+) сек.") then
				local sec = text:match(reason .. "(%d+) сек.")
	...
	if text:find("Вы не отбываете наказание в тюрьме.") then
		jailTimerState = false
		return false
	end
	...
	if text:find("Вы не разыскиваетесь ни в одном из государств.") then
		wanted.us, wanted.rc, wanted.af = 0, 0, 0
		return false
	end

	if text:find("Ваш текущий уровень розыска в %u%u составляет %d+ .+") then
...
function sampev.onShowDialog(id, style, title, btn1, btn2, text)
	...
	if id == 45 and title:find("Информация о вашем розыске, всего %d+") then
